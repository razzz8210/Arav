...
export async function POST(request: Request) {
  try {
    const { session_id, media_items, slideshow_name, background_music_url } =
      await request.json();
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/generate_slideshow/${encodeURIComponent(
        session_id
      )}`,
      {
        method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: (data as any)?.error || "Failed to create slideshow" },
